...
# [1] ---- Load global data ----
    ...
    pal_education_1 <- colorNumeric(palette = c("white","purple4"),domain = map$education_score)
    pal_green_1 <- colorNumeric(palette = c("white","darkgreen"),domain = map$green_score)
    pal_green_2 <- colorNumeric(palette = c("white","darkgreen"),domain = map$green_score_decile)
    pal_transport_1 <- colorNumeric(palette = c("white","darkorange"),domain = map$public_transport_proportion)
    pal_transport_2 <- colorNumeric(palette = c("white","darkorange"),domain = map$motor_vehicle_proportion)
    pal_transport_3 <- colorNumeric(palette = c("white","darkorange"),domain = map$bicycle_walking_proportion)
    pal_census_1 <- colorNumeric(palette = c("white","navy"),domain = map$usual_resident_population)
    pal_census_2 <- colorNumeric(palette = c("white","navy"),domain = map$working_age_proportion)
    pal_census_3 <- colorNumeric(palette = c("white","navy"),domain = map$senior_citizen_proportion)
    pal_seifa_1 <- colorNumeric(palette = c("white","navy"),domain = map$seifa_econ_disadvantage)
    pal_seifa_2 <- colorNumeric(palette = c("white","navy"),domain = map$seifa_econ_adv_disadv)
    pal_seifa_3 <- colorNumeric(palette = c("white","navy"),domain = map$seifa_econ_resources)
    pal_seifa_4 <- colorNumeric(palette = c("white","navy"),domain = map$seifa_education_occupation)
    pal_housing_1 <- colorNumeric(palette = c("white","saddlebrown"),domain = map$house_and_semi_proportion)
    pal_housing_2 <- colorNumeric(palette = c("white","saddlebrown"),domain = map$unit_proportion)
    pal_housing_3 <- colorNumeric(palette = c("white","saddlebrown"),domain = map$dwelling_density)
    pal_prices_1 <- colorNumeric(palette = c("white","deeppink4"),domain = map$median_land_value_per_sqm)
    pal_prices_2 <- colorNumeric(palette = c("white","deeppink4"),domain = map$house_median_suburb)
    pal_prices_3 <- colorNumeric(palette = c("white","deeppink4"),domain = map$apartment_median_suburb)
    pal_aria_1 <- colorNumeric(palette = c("white","darkred"),domain = map$aria_overall)
    pal_aria_2 <- colorNumeric(palette = c("white","darkred"),domain = map$aria_education)
    pal_aria_3 <- colorNumeric(palette = c("white","darkred"),domain = map$aria_health)
    pal_aria_4 <- colorNumeric(palette = c("white","darkred"),domain = map$aria_shopping)
    pal_aria_5 <- colorNumeric(palette = c("white","darkred"),domain = map$aria_public_transport)
    pal_aria_6 <- colorNumeric(palette = c("white","darkred"),domain = map$aria_financial_postal)
            ...
            if(input$variable == "Crime"){
                ...
                clearShapes() %>%
                    addPolygons(data = map,
                                ...
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,8:10), feature.id = FALSE, row.numbers = FALSE),
                                group = "Crime Score") %>%
                    addPolygons(data = map,
                                weight = 1, 
                ...
                leafletProxy("map") %>%
                    ...
                    addPolygons(data = map,
                                ...
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,11), feature.id = FALSE, row.numbers = FALSE),
                                group = "Education") %>%
                ...
                leafletProxy("map") %>%
                    ...
                    addPolygons(data = map,
                                weight = 1, 
                                fillColor = ~pal_green_1(green_score), 
                                color = "black",
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,12,13), feature.id = FALSE, row.numbers = FALSE),
                                group = "Green Space - Score") %>%
                    addPolygons(data = map,
                                weight = 1, 
                                fillColor = ~pal_green_2(green_score_decile), 
                                color = "black",
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,12,13), feature.id = FALSE, row.numbers = FALSE),
                                group = "Green Space - Decile") %>%
                    addLayersControl(baseGroups = c("Green Space - Score",
                                                    "Green Space - Decile"),
                                     options = layersControlOptions(collapsed = FALSE),
                                     position = "topright")
            } 
                ...
                leafletProxy("map") %>%
                    ...
                    addPolygons(data = map,
                                ...
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,18:20), feature.id = FALSE, row.numbers = FALSE),
                ...
                leafletProxy("map") %>%
                    ...
                    addPolygons(data = map,
                                ...
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,14:16), feature.id = FALSE, row.numbers = FALSE),
                                group = "Population") %>%
                    addPolygons(data = map,
                                weight = 1, 
                                ...
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,14:26), feature.id = FALSE, row.numbers = FALSE),
                ...
                leafletProxy("map") %>%
                    ...
                    addPolygons(data = map,
                                ...
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,25:28), feature.id = FALSE, row.numbers = FALSE),
                ...
                leafletProxy("map") %>%
                    ...
                    addPolygons(data = map,
                                ...
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,22:24), feature.id = FALSE, row.numbers = FALSE),
                ...
                leafletProxy("map") %>%
                    ...
                    addPolygons(data = map,
                                ...
                                opacity = 1,
                                fillOpacity = 0.8,
                                popup = leafpop::popupTable(map_data, zcol = c(2,4:6,31:36), feature.id = FALSE, row.numbers = FALSE),
                                group = "ARIA - Overall") %>%
                    addPolygons(data = map,
                                weight = 1, 
